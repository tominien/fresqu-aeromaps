...
from core.parse_process_data import get_years, calculate_aspect_output_value




# Load the lines (historic, no aspect and all aspect) from the JSON file :
LINES_JSON_PATH = Path(__file__).resolve().parents[2] / "data" / "prospective_scenario_lines.json"
LINES                              = get_aspects(LINES_JSON_PATH)
LINES_NAMES: List[str]             = get_aspects_names(LINES_JSON_PATH)
LINES_OUTPUT_FORMULAS: List[str]   = [line["output_formula"] for line in LINES.values()]

# Load the aspects from the JSON file :
ASPECTS                            = get_aspects()
ASPECTS_NAMES: List[str]           = get_aspects_names()
ASPECTS_OUTPUT_FORMULAS: List[str] = [aspect["output_formula"] for aspect in ASPECTS.values()]
...
class ProspectiveScenarioGraph(BaseGraph):
    ...
    def draw(
        ...
        historic_y_line = calculate_aspect_output_value(
            process_data,
            LINES_OUTPUT_FORMULAS[0]
        )
        ...
        prospective_y_lines = [
            calculate_aspect_output_value(process_data, aspect_output_formula)
            for aspect_output_formula in LINES_OUTPUT_FORMULAS[1:3]
        ]
        ...
        colors = [self.color_palette[i] for i in indices]
        self._aspects_areas = Lines(
            ...
            fill = 'between',
            fill_colors = colors,
            fill_opacities = [0.3] * (len(colors) - 1),
            labels = ASPECTS_NAMES,
            display_legend = True,
            scales = {"x": x_scale, "y": y_scale, "color": color_scale}
        )
    ...
    def update(self, process_data: Dict[str, Any]) -> Figure:
        ...
        prospective_lines_no_aspects_line  = calculate_aspect_output_value(process_data, LINES_OUTPUT_FORMULAS[1])
        prospective_lines_all_aspects_line = calculate_aspect_output_value(process_data, LINES_OUTPUT_FORMULAS[2])
        self._prospective_lines.y = [
            prospective_lines_no_aspects_line,
            prospective_lines_all_aspects_line
        ]
        ...
        self._prospective_labels.y = [
            prospective_lines_no_aspects_line.iloc[-1],
            prospective_lines_all_aspects_line.iloc[-1]
        ]
        self._prospective_labels.text = [
            float_to_int_string(prospective_lines_no_aspects_line.iloc[-1]),
            float_to_int_string(prospective_lines_all_aspects_line.iloc[-1])
        ]
